        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/accounts.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      ...
      if (this.pageType === 'view') {
        this.$axios
          ...
          .then(({ data }) => {
            this.viewEditData = data.account
            data.account.active
              ? (this.radio = 'Enabled')
              : (this.radio = 'Disabled')
            this.isLoading = !this.isLoading
    ...
    saveAction() {
      ...
      if (this.pageID && this.pageType === 'edit') {
        account = {
          ...
          branch: {
            id:
              this.allInputAndLookUpValue?.branchName ??
              this.viewEditData?.branchId ??
              '',
          },
          currency: {
            id:
              this.allInputAndLookUpValue?.currencyName ??
              this.viewEditData?.currencyId ??
              '',
          },
          iban:
            this.allInputAndLookUpValue?.iban ?? this.viewEditData?.iban ?? '',
          name:
            this.allInputAndLookUpValue?.name ?? this.viewEditData?.name ?? '',
          swift:
            this.allInputAndLookUpValue?.swift ??
            this.viewEditData?.swift ??
            '',
          active:
        ...
        account = {
          ...
          branch: { id: this.allInputAndLookUpValue?.branchName ?? '' },
          currency: { id: this.allInputAndLookUpValue?.currencyName ?? '' },
          iban: this.allInputAndLookUpValue?.iban ?? '',
          name: this.allInputAndLookUpValue?.name ?? '',
          swift: this.allInputAndLookUpValue?.swift ?? '',
      ...
      this.$axios[method](
        `/account/${this.pageID ? 'editAccount' : 'addAccount'}`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
          this.$router.push('/accounts.htm')
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
          // eslint-disable-next-line no-console
    ...
    dataCreatedAction() {
      const data = [
        ...
        {
          name: 'Currency',
          subName: 'currencyName',
          selectName: 'currencies',
          api: 'findAllCurrency',
          type: 'select',
          show: true,
          disabled: this.pageType === 'view',
