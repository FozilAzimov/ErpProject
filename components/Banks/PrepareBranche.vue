        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/branches.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      ...
      if (this.pageType === 'view') {
        this.$axios
          .post(`/branch/prepareBranche`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
          .then(({ data }) => {
            this.viewEditData = data.branch
            data.branch.active
              ? (this.radio = 'Enabled')
              : (this.radio = 'Disabled')
            this.isLoading = !this.isLoading
    ...
    saveAction() {
      ...
      if (this.pageID && this.pageType === 'edit') {
        branch = {
          ...
          bank: {
            id:
              this.allInputAndLookUpValue?.bankName ??
              this.viewEditData?.bankId ??
              '',
          },
          name:
        ...
        branch = {
          bank: { id: this.allInputAndLookUpValue?.bankName ?? '' },
      ...
      this.$axios[method](
        `/branch/${this.pageID ? 'editBranche' : 'addBranche'}`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
