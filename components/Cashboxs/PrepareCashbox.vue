        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/cashboxs.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
...
export default {
  ...
  methods: {
      ...
      if (this.pageType === 'view') {
        this.$axios
          ...
          .then(({ data }) => {
            ...
            this.viewEditData = data.cashbox
            data.cashbox.active
              ? (this.radio = 'Enabled')
              : (this.radio = 'Disabled')
            this.isLoading = !this.isLoading
    ...
    saveAction() {
      ...
      this.$axios[method](
        `/cashbox/${this.pageID ? 'editCashbox' : 'addCashbox'}`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
          this.$router.push('/cashboxs.htm')
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
          // eslint-disable-next-line no-console
    ...
    dataCreatedAction() {
      const data = [
        ...
        {
          name: 'department.telegramBotChatId',
        ...
        {
          name: 'department.telegramReportType',
