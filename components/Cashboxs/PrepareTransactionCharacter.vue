      ...
      >
        <div class="flex items-center gap-[10px]">
          ...
          <h1 class="font-bold text-[rgb(49,126,172)] text-[14px] uppercase">
            {{
              pageType === 'view'
                ? 'View TRANSACTION CHARACTER'
                : pageType === 'edit'
                ? 'Edit TRANSACTION CHARACTER'
                : 'Add TRANSACTION CHARACTER'
            }}
          </h1>
        </div>
        <div>
        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/transactionCharacters.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      ...
      if (this.pageType === 'view') {
        this.$axios
          .post(`/transactionCharacter/prepareTransactionCharacter`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
          .then(({ data }) => {
            this.viewEditData = data.transactionCharacter
            data.transactionCharacter.active
              ? (this.radio = 'Enabled')
              : (this.radio = 'Disabled')
            this.isLoading = !this.isLoading
          })
    ...
    saveAction() {
      ...
      this.$axios[method](
        `/transactionCharacter/${
          this.pageID ? 'editTransactionCharacter' : 'addTransactionCharacter'
        }`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
          this.$router.push('/transactionCharacters.htm')
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
          // eslint-disable-next-line no-console
