        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/articles.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      if (this.pageType === 'view') {
        ...
        this.$axios
          .post(`/articles/prepareArticle`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
          .then(({ data: { article } }) => {
            this.viewEditData = article
            article.active
              ? (this.radio = 'Enabled')
              : (this.radio = 'Disabled')
            this.isLoading = !this.isLoading
    ...
    saveAction() {
      ...
      this.$axios[method](
        `/articles/${this.pageID ? 'editArticle' : 'addArticle'}`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
          this.$router.push('/articles.htm')
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
          // eslint-disable-next-line no-console
