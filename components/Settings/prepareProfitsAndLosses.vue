        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/profitsAndLosses.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      if (this.pageType === 'edit') {
        ...
        this.$axios
          .get(`/profitsAndLosses/findProfitsAndLosses/${this.pageID}`)
          .then(({ data }) => {
            this.editData = data
            this.isLoading = !this.isLoading
          })
    ...
    saveAction() {
      if (this.allInputAndLookUpValue?.amount || this.editData?.amount) {
        ...
        this.$axios[method](
          `/profitsAndLosses/${
            this.pageID ? `updateProfitsAndLosses` : 'createProfitsAndLosses'
          }`,
          body
        )
          .then(() => {
            this.isLoading = !this.isLoading
            this.$router.push('/profitsAndLosses.htm')
          })
          .catch((error) => {
            this.isLoading = !this.isLoading
            // eslint-disable-next-line no-console
