        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/manufacturers.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      if (this.pageType === 'view') {
        ...
        this.$axios
          .post(`/manufacturer/prepareManufacturer`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
          .then(({ data: { manufacturer } }) => {
            this.viewEditData = manufacturer
            manufacturer.active
              ? (this.radio = 'Enabled')
              : (this.radio = 'Disabled')
            this.isLoading = !this.isLoading
    ...
    saveAction() {
      ...
      if (this.pageID && this.pageType === 'edit') {
        manufacturer = {
          ...
          active:
            ...
            false,
          arrow:
            this.allInputAndLookUpValue?.arrow ??
            this.viewEditData?.arrow ??
            false,
        }
      } else {
        manufacturer = {
          ...
          arrow: this.allInputAndLookUpValue?.arrow ?? false,
        }
      }
      body = {
      ...
      this.$axios[method](
        `/manufacturer/${this.pageID ? 'editManufacturer' : 'addManufacturer'}`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
          this.$router.push('/manufacturers.htm')
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
          // eslint-disable-next-line no-console
