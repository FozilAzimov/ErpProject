  ...
  methods: {
    ...
    getTableRequest() {
      if (this.pageType === 'view') {
        ...
        this.$axios
          .post(`/productionArticle/prepareProductionArticleViewAjaxLoad`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
    ...
    saveAction() {
      ...
      this.$axios[method](
        `/productionArticle/${
          this.pageID ? 'editProductionArticle' : 'addProductionArticle'
        }`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
          this.$router.push('/productionArticles.htm')
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
          // eslint-disable-next-line no-console
