        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/obligations.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      if (this.pageType === 'edit') {
        ...
        this.$axios
          .get(`/obligations/findObligations/${this.pageID}`)
          .then(({ data }) => {
            this.editData = data
            this.isLoading = !this.isLoading
          })
    ...
    saveAction() {
      if (this.allInputAndLookUpValue?.amount || this.editData?.amount) {
        ...
        this.$axios[method](
          `/obligations/${
            this.pageID ? `updateObligations` : 'createObligations'
          }`,
          body
        )
          .then(() => {
            this.isLoading = !this.isLoading
            this.$router.push('/obligations.htm')
          })
          .catch((error) => {
            this.isLoading = !this.isLoading
            // eslint-disable-next-line no-console
