        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/plateNumber.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      if (this.pageType === 'view') {
        ...
        this.$axios
          .post(`/plateNumber/preparePlateNumberView`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
          .then(({ data: { carName } }) => {
            this.isLoading = !this.isLoading
            this.viewEditData.name = carName
          })
          .catch((error) => {
            this.isLoading = !this.isLoading
        ...
        this.isLoading = !this.isLoading
        this.$axios
          .post(`/plateNumber/preparePlateNumber`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
          .then(({ data }) => {
            this.isLoading = !this.isLoading
            this.planteNumberList = data?.planteNumbers
            this.viewEditData = data
          })
          .catch((error) => {
            this.isLoading = !this.isLoading
            // eslint-disable-next-line no-console
        ...
        this.isLoading = !this.isLoading
        this.$axios
          .post(`/plateNumber/prepareAddPlateNumber`, {
            page_current: 1,
            page_size: 25,
          })
          .then(({ data: { planteNumber } }) => {
            this.isLoading = !this.isLoading
            this.planteNumberList = planteNumber
          })
          .catch((error) => {
            this.isLoading = !this.isLoading
            // eslint-disable-next-line no-console
    ...
    saveAction() {
      ...
      if (this.pageID && this.pageType === 'edit') {
        plateNumber = {
          id: this.pageID,
          name:
            this.allInputAndLookUpValue?.plateName ??
            this.viewEditData?.plateName ??
            '',
          car: {
            id:
              this.allInputAndLookUpValue?.carName ??
              this.viewEditData?.carId ??
              '',
          },
        }
      } else {
        plateNumber = {
          car: {
            id: this.allInputAndLookUpValue?.carName ?? '',
          },
          name: this.allInputAndLookUpValue?.plateName,
        }
      }
      body = {
      ...
      this.$axios[method](
        `/plateNumber/${this.pageID ? 'editPlateNumber' : 'addPlateNumber'}`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
          this.$router.push('/plateNumber.htm')
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
          // eslint-disable-next-line no-console
