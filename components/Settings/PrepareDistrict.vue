        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/district.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      ...
      if (this.pageType === 'view') {
        this.$axios
          .post(`/district/districtView`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
          .then(({ data: { district } }) => {
            this.viewEditData = district
            this.isLoading = !this.isLoading
          })
          .catch((error) => {
            this.isLoading = !this.isLoading
        ...
        this.$axios
          .post(`/district/prepareDistrict`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
          .then(({ data: { district, city } }) => {
            this.viewEditData = district
            this.selectData.city = city
            this.isLoading = !this.isLoading
          })
          .catch((error) => {
            this.isLoading = !this.isLoading
    ...
    saveAction() {
      if (this.allInputAndLookUpValue?.name || this.viewEditData?.name) {
        ...
        this.$axios[method](
          `/district/${this.pageID ? 'editDistrict' : 'addDistrict'}`,
          body
        )
          .then(() => {
            this.isLoading = !this.isLoading
            this.$router.push('/district.htm')
          })
          .catch((error) => {
            this.isLoading = !this.isLoading
            // eslint-disable-next-line no-console
