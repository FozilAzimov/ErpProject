        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/productEntryTypes.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      ...
      if (this.pageType === 'view') {
        this.$axios
          .post(`/productEntry/prepareProductEntryType`, {
            id: this.pageID,
            page_current: 1,
            page_size: 25,
          })
          .then(({ data: { producEntryType } }) => {
            this.viewEditData = producEntryType
            producEntryType.active
              ? (this.radio = 'Enabled')
              : (this.radio = 'Disabled')
            this.isLoading = !this.isLoading
          })
    ...
    saveAction() {
      ...
      this.$axios[method](
        `/productEntry/${
          this.pageID ? 'editProductEntryType' : 'addProductEntryType'
        }`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
          this.$router.push('/productEntryTypes.htm')
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
          // eslint-disable-next-line no-console
