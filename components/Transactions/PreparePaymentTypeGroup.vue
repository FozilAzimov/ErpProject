      ...
      >
        <div class="flex items-center gap-[10px]">
          ...
          <h1 class="font-bold text-[rgb(49,126,172)] text-[14px] uppercase">
            {{
              pageType === 'view'
                ? 'View Payment Type Group'
                : pageType === 'edit'
                ? 'Edit Payment Type Group'
                : 'Add Payment Type Group'
            }}
          </h1>
        </div>
        <div>
        ...
        <div class="w-fit flex flex-col items-start m-2 gap-1">
          ...
          <div class="flex items-center gap-3 mt-3">
            <generic-button
              ...
              @click="$router.push('/paymenttypegroups.htm')"
            />
            <generic-button
              v-if="pageType !== 'view'"
  ...
  methods: {
    ...
    getTableRequest() {
      if (this.pageType === 'view' || this.pageType === 'edit') {
        ...
        this.$axios
          .get(`/paymentGroup/findById/${this.pageID}`)
          .then(({ data }) => {
            this.viewEditData = data
            this.radio = data.active ? 'Enabled' : 'Disabled'
            this.isLoading = !this.isLoading
          })
          .catch((error) => {
            this.isLoading = !this.isLoading
    ...
    saveAction() {
      ...
      this.$axios[method](
        `/paymentGroup/${this.pageID ? 'update' : 'save'}`,
        body
      )
        .then(() => {
          this.isLoading = !this.isLoading
          this.$router.push('/paymenttypegroups.htm')
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
          // eslint-disable-next-line no-console
