        ...
        async setNewValue(value, objKey){
            ...
            if(objKey === 'currencyKey'){
                if(this.defProperties){
                    const data = {
                        branchCompanyId: this.defaultProps.branchCompanyId ? this.defaultProps.branchCompanyId : null,
                        currencyId: this.valueObject.currencyKey.id ? this.valueObject.currencyKey.id : null,
                        dateFrom: this.defaultProps.dateFrom ? this.defaultProps.dateFrom : null,
                        employeeId: this.defaultProps.employeeId ? this.defaultProps.employeeId : null,
                        settingsRateType: this.defaultProps.settingsRateType ? this.defaultProps.settingsRateType : null,

                    }

                    try{
                        const response = await this.$axios.post('invoiceBase/getCurrentCurrencyRate', data, 
                            {
                                headers: {
                ...
                console.log(this.newResponseData, 'this.newResponseData');
                if(this.newResponseData){
