    ...
    >
      <div class="flex items-center gap-[10px]">
        <generic-button
          ...
          @click="$router.push('/outputToProductionCompany.htm')"
        />
        <h1 class="font-bold text-[rgb(49,126,172)] text-[14px] uppercase">
          {{
            pageID
              ? 'Edit Output To Production Company'
              : 'Add Output To Production Company'
          }}
        </h1>
      </div>
    </div>
    <div class="border-[1px] border-solid border-[rgba(0,0,0,0.1)]">
      ...
      <div v-if="isAccept || pageID" class="m-1">
        ...
        <GenericPrepareTablePage
          ...
          tab-name="outputToProductionCompanyItemTable"
          filter-type="max"
...
export default {
  ...
  methods: {
    ...
    getTableRequest(pageID) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareOutputToProductionCompanyNew`, body)
    ...
    getRowElements(arr, hideBtn) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareCreateEditOutputToProductionCompany`, body)
        .then(({ data: { invoiceJson } }) => {
          this.editData = invoiceJson
          this.bodyData = invoiceJson?.invoiceItems
          if (!this.pageID) {
            this.$router.push(
              `prepareOutputToProductionCompanyNew.htm/${invoiceJson?.id}`
            )
          }
