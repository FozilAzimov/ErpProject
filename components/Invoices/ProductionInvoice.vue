    ...
    <template v-if="isCloseTable">
      ...
      >
        <div class="flex items-center gap-1 m-2">
          <generic-button
            v-for="(element, index) in createBtnData"
            :key="index"
            :name="element.name"
            type="primary"
            :icon-name-attribute="
              element.btnType !== 'five' ? 'circle-plus-outline' : ''
            "
            @click="
              $router.push({
                path: element?.path,
        ...
        <div class="p-2">
          ...
          <GenericTablePage
            ...
            open-url="prepareProductionInvoiceNew"
            height="600"
          />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getTableRequest() {
      ...
      this.$axios
        .post(`/invoices/productioninvoice`, body)
        .then(({ data: { response } }) => {
          ...
          // function
          this.getTableBody(response?.invoiceList)
          this.isLoading = !this.isLoading
        })
        .catch((error) => {
          this.isLoading = !this.isLoading
    ...
    createDataFiltering() {
      ...
      this.topFilterData = createData

      const createBtn = [
        {
          name: 'Production Internal Invoice',
          btnType: 'one',
          path: 'prepareProductionInvoiceNew.htm',
          query: {
            internalStatus: true,
          },
        },
