    ...
    >
      <div class="flex items-center gap-[10px]">
        <generic-button
          ...
          @click="$router.push('/expenseInvoice.htm')"
        />
        <h1 class="font-bold text-[rgb(49,126,172)] text-[14px] uppercase">
          {{ pageID ? 'Edit Expense Invoice' : 'Add Expense Invoice' }}
        </h1>
      </div>
    </div>
    <div class="border-[1px] border-solid border-[rgba(0,0,0,0.1)]">
      ...
      <div v-if="isAccept || pageID" class="m-1">
        ...
        <GenericPrepareTablePage
          ...
          tab-name="expenseInvoiceItemTable"
          filter-type="max"
...
export default {
  ...
  methods: {
    ...
    getTableRequest(pageID) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareExpenseInvoice`, body)
