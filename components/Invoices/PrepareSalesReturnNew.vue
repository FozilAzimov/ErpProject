    ...
    >
      <div class="flex items-center gap-[10px]">
        <generic-button
          ...
          @click="$router.push('/salesReturn.htm')"
        />
        <h1 class="font-bold text-[rgb(49,126,172)] text-[14px] uppercase">
          {{ pageID ? 'Edit salesReturn' : 'Add salesReturn' }}
        </h1>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getTableRequest(pageID) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareSalesReturnNew`, body)
        .then(
          ({
    ...
    getDisabledValue(disabledVal, tabName) {
      if (tabName === 'salesReturnItemTable') this.saveBtnDisabled = disabledVal
    ...
    getRowElements(arr, hideBtn) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareCreateEditSalesReturn`, body)
        .then(({ data: { invoiceJson } }) => {
          this.allSelectAndInputValue = invoiceJson
          this.bodyData = invoiceJson?.invoiceItems
          if (!this.pageID) {
            this.$router.push(`prepareSalesReturnNew.htm/${invoiceJson?.id}`)
          }
