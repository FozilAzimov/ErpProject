    ...
    >
      <div class="flex items-center gap-[10px]">
        ...
        <h1 class="font-bold text-[rgb(49,126,172)] text-[14px] uppercase">
          {{ pageID ? 'Edit Purchase Return' : 'Add Purchase Return' }}
        </h1>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getTableRequest(pageID) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareInputReturnNew`, body)
        .then(
          ({
    ...
    getDisabledValue(disabledVal, tabName) {
      if (tabName === 'inputReturnItemTable') this.saveBtnDisabled = disabledVal
    ...
    getRowElements(arr, hideBtn) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareCreateEditInputReturn`, body)
        .then(({ data: { invoiceJson } }) => {
          this.editData = invoiceJson
          this.bodyData = invoiceJson?.invoiceItems
          if (!this.pageID) {
            this.$router.push(`prepareInputReturnNew.htm/${invoiceJson?.id}`)
          }
