  ...
  <div class="w-full p-[0px_10px_0px_4px]">
    ...
    >
      <div class="flex items-center gap-[10px]">
        <generic-button
          ...
          @click="$router.push('/purchaseServiceInvoice.htm')"
        />
        <h1 class="font-bold text-[rgb(49,126,172)] text-[14px] uppercase">
          {{ pageID ? 'Edit Purchase Service' : 'Add Purchase Service' }}
        </h1>
      </div>
    </div>
    <div class="border-[1px] border-solid border-[rgba(0,0,0,0.1)] mb-10">
      ...
      <div v-if="pageID" class="m-1">
        ...
        <div class="flex gap-1 flex-wrap">
          ...
          <GenericButton
            ...
            type="info"
            name="Undo Payment"
            @click="undoPaymentAction('sub_table')"
          />
          <GenericButton
            v-else
            name="Edit"
        ...
        <GenericPrepareTablePage
          ...
          :addmodalorrow="false"
          tab-name="subCustomTable"
          :tablehead="getFilterSubAndExtraSubTableHeadData"
          :tableheadlength="getFilterSubAndExtraSubTableHeadData?.length"
      ...
      <div v-if="pageID" class="m-1">
        ...
        >
        <div class="flex gap-1 flex-wrap">
          <span v-if="extraSubHideButton" class="flex gap-1 flex-wrap">
            <GenericButton
              name="Pay"
              type="primary"
              :disabled="extraSubSaveBtnDisabled"
              @click="saveAction('extra_sub_table')"
            />
            <GenericButton
              v-if="pageID"
              name="Discard"
              @click="discardAction('extra_sub_table')"
            />
          </span>
          <GenericButton
            v-else-if="!extraSubHideButton && isExtraSubBodyDataLength"
            type="info"
            name="Undo Payment"
            @click="undoPaymentAction('extra_sub_table')"
          />
          <GenericButton
            v-else
            name="Edit"
            type="success"
            icon-name-attribute="edit"
            @click="editAction('extra_sub_table')"
          />
        </div>
        <GenericPrepareTablePage
          ref="extraSubCustomTableRef"
          :addmodalorrow="false"
          tab-name="extraSubCustomTable"
          :tablehead="getFilterSubAndExtraSubTableHeadData"
          :tableheadlength="getFilterSubAndExtraSubTableHeadData?.length"
          :response-data="extraSubBodyData"
          :ui-show-hide="extraSubUiShowHide"
          :is-edit="extraSubIsEdit"
          :height="450"
          delete-url-row=""
          class="bg-[rgba(255,255,255,0.5)] mt-1"
          @rowValues="getExtraSubRowElements"
          @requiredAction="getDisabledValue"
        />
      </div>
      <!-- --END-- Extra_Sub Custom Table -->
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      // sub Table uchun
      // extra sub Table uchun
      extraSubBodyData: [],
      extraSubUiShowHide: false,
      extraSubIsEdit: null,
      extraSubHideButton: null,
      isExtraSubBodyDataLength: false,
      extraSubSaveBtnDisabled: false,
      // extra sub Table uchun
    }
  },
  ...
  created() {
    ...
    // sub table uchun
    // extra sub table uchun
    this.extraSubIsEdit = !!this.pageID
    this.extraSubHideButton = !this.pageID
    this.extraSubUiShowHide = !!this.pageID
    // extra sub table uchun
  },
  ...
  methods: {
    ...
    getTableRequest(pageID) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/preparePurchaseServiceInvoiceNew`, body)
        .then(
          ({
            data: {
              actionUrl,
            ...
            this.subAndExtraSubHeadData = transactionsColumns
            // sub table uchun
            this.subBodyData = invoiceJson?.transactions
            invoiceJson?.transactions.length
              ? (this.isSubBodyDataLength = true)
              : (this.isSubBodyDataLength = false)
            // sub table uchun
            // extra sub table uchun
            this.extraSubBodyData = invoiceJson?.transactionsExtraList
            invoiceJson?.transactionsExtraList.length
              ? (this.isExtraSubBodyDataLength = true)
              : (this.isExtraSubBodyDataLength = false)
            // extra sub table uchun
            this.isLoading = !this.isLoading
          }
        )
    ...
    editAction(propName) {
      ...
      } else if (propName === 'extra_sub_table') {
        this.extraSubHideButton = !this.extraSubHideButton
        // GenericTablePage da ishlab beruvchi function
        this.$refs.extraSubCustomTableRef.getEditRowAction(this.pageID)
        this.extraSubUiShowHide = false
      }
    },
    ...
    discardAction(propName) {
      if (propName === 'top_table') {
        ...
        // GenericTablePage da ishlab beruvchi function
        this.$refs.extraSubCustomTableRef.discardArrayFiltered()
        this.extraSubBodyData = this.extraSubBodyData.filter((obj) => obj?.id)
        this.extraSubHideButton = !this.extraSubHideButton
        this.extraSubUiShowHide = true
      }
    },

    // Undo Payment Action
    undoPaymentAction(propName) {
      this.isLoading = !this.isLoading
      this.$axios
    ...
    getDisabledValue(disabledVal, tabName) {
      ...
      else if (tabName === 'subCustomTable')
        this.subSaveBtnDisabled = disabledVal
      else if (tabName === 'extraSubCustomTable')
        this.extraSubSaveBtnDisabled = disabledVal
    },
    ...
    getRowElements(arr, hideBtn) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareCreateEditPurchaseServiceInvoice`, body)
        .then(({ data }) => {
          this.editData = data
          this.bodyData = data?.invoiceItems
          if (!this.pageID) {
            this.$router.push(
              `preparePurchaseServiceInvoiceNew.htm/${data?.id}`
            )
          }
    ...
    // EMIT action EXTRA_SUB_table
    getExtraSubRowElements(arr, hideBtn) {
      // function
      this.extraSubBodyData = this.staticSetDataAction(arr)

      // Start Request body
      const body = {}
      body.id = this.pageID
      body.transactionsExtraList = this.extraSubBodyData
      // End Request body
      ...
      this.$axios
        ...
        .then(({ data }) => {
          this.extraSubBodyData = data?.orderItems
          this.extraSubUiShowHide = true
          this.extraSubHideButton = !hideBtn
          // function
          this.getTableRequest(this.pageID)
          this.isLoading = !this.isLoading
          this.$notification(`Ma'lumot saqlandi!`, 'Success', 'success')
    ...
    saveAction(propName) {
      ...
      if (propName === 'top_table') {
        ...
        this.$refs.extraSubCustomTableRef.getSaveRowAction()
      }
    },
      ...
      if (propMessage === 'confirm') {
        this.$axios
          ...
          .then(() => {
            ...
            this.$router.push('/purchaseServiceInvoice.htm')
            this.isLoading = !this.isLoading
          })
          .catch((error) => {
        ...
        [
          ...
          },
          {
            ...
            param: {
              branchcompany: true,
              companyGroupId:
                this.allSelectAndInputValue?.companyGroupId ?? null,
            },
            type: 'select',
            required: true,
          },
