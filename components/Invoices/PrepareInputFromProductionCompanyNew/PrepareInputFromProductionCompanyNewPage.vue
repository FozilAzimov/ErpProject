    ...
    >
      <div class="flex items-center gap-[10px]">
        <generic-nuxt-link-button
          ...
          :url="img.goBack"
          :istherepicture="true"
          to="/inputFromProductionCompany.htm"
        />
...
export default {
  ...
  methods: {
    ...
    getPageRequest() {
      ...
      this.$axios
        .post(
          `/invoices/prepareInputFromProductionCompanyNew`,
    ...
    getResponseTransactionsAction(prop) {
      this.$axios
        .post(
          `/invoices/prepareInputFromProductionCompanyNewAjaxLoad`,
          { id: this.parentID ? this.parentID : this.userId },
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
    ...
    saveInvoice() {
      if (!this.disabledButton) {
        ...
        this.$axios
          .post(
            `/invoices/prepareCreateEditInputFromProductionCompany`,
