    ...
    >
      <div class="flex items-center gap-[10px]">
        ...
        <h1 class="font-bold text-[rgb(49,126,172)] text-[14px] uppercase">
          {{ pageID ? 'Edit Sale Service' : 'Add Sale Service' }}
        </h1>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getTableRequest(pageID) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareSaleServiceInvoiceNew`, body)
        .then(
          ({
            data: {
              actionUrl,
    ...
    getRowElements(arr, hideBtn) {
      ...
      this.isLoading = !this.isLoading
      this.$axios
        .post(`/invoices/prepareCreateEditSaleServiceInvoice`, body)
        .then(({ data }) => {
          this.editData = data
          this.bodyData = data?.invoiceItems
          if (!this.pageID) {
            this.$router.push(`prepareSaleServiceInvoiceNew.htm/${data?.id}`)
          }
