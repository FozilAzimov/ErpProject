      ...
      >
        <div class="p-3 flex items-center gap-5">
          ...
          <generic-button
            ...
            disabled-bg="rgba(54, 155, 215, 0.4)"
...
export default {
  ...
  methods: {
    ...
    getLookUpValue(name, valueText, valueId) {
      this.allLookUpValues[name] = valueId
    },

    // Generic_Date value
    getInputDateValues(name, value) {
      const [day, month, year, time] = value.split(/[-/T]+/)
      this.allLookUpValues[name] = `${year}/${month}/${day} ${time}:00`
    },
    ...
    acceptAction() {
      if (
        ...
        this.allLookUpValues.warehouseId &&
        this.allLookUpValues.invoiceTypeId
      ) {
        this.isLoading = !this.isLoading
        this.$axios
