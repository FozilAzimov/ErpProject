      ...
      >
        <div class="flex items-center m-2 gap-1">
          <generic-button
            ...
            @click="
              $router.push({
                path: 'prepareSaleInvoiceNew.htm',
                query: { saleToPerson: false },
              })
            "
          />
          <generic-button name="Add New To Device" type="primary" />
          <generic-button
...
export default {
  ...
  methods: {
    ...
    getTableRequest() {
      ...
      this.$axios
        .post(`/invoices/saleinvoice`, body)
        .then(
          ({
            data: {
              invoiceSearchDTO,
              InvoiceDTO: { actionUrl, invoiceList, rightMap, leftMap },
            },
          }) => {
            this.actionUrl = actionUrl
            this.leftMap = leftMap
            this.tableHead = rightMap
            this.selectData.payStatusList = invoiceSearchDTO.payStatusList
            this.selectData.billStatusList = invoiceSearchDTO.billStatusList
            // function
            this.getTableBody(invoiceList)
            this.isLoading = !this.isLoading
          }
        )
        .catch((error) => {
