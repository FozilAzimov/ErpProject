    ...
    <template v-if="isCloseTable">
      ...
      >
        <div class="flex items-center gap-2 m-2">
          <generic-button
            name="Production Internal Invoice"
            type="primary"
            @click="goToPageAction('yes_internalStatus')"
          />
          <generic-button
            name="Production on Internal"
            type="primary"
            @click="goToPageAction('not_internalStatus')"
          />
          <generic-button
            name="Production on Internal yarn"
            type="primary"
            @click="goToPageAction('productionInvoiceType')"
          />
          <generic-button
            name="Production on Internal Woven"
            type="primary"
            @click="goToPageAction('productionInvoiceTypeWoven')"
          />
          <generic-button
            name="Production for Qr"
            type="primary"
            @click="$router.push('/productionForQrCodeList.htm')"
          />
        </div>
        <div class="p-2">
  ...
  methods: {
    goToPageAction(prop) {
      if (prop === 'yes_internalStatus') {
        this.$store.commit('SET_INTERNAL_STATUS', true)
      } else if (prop === 'not_internalStatus') {
        this.$store.commit('SET_INTERNAL_STATUS', false)
      } else if (prop === 'productionInvoiceType') {
        this.$store.commit('SET_PRODUCTION_INVOICE_TYPE', true)
      } else if (prop === 'productionInvoiceTypeWoven') {
        this.$store.commit('SET_PRODUCTION_INVOICE_TYPE_WOVEN', true)
      }

      this.$router.push('/prepareProductionInvoiceNew.htm')
    },
    ...
    getTableRequest() {
      ...
      this.$axios
        .post(`/invoices/productioninvoice`, {
          current_page: 1,
