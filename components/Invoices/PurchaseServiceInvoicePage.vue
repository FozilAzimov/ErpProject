    ...
    <template v-if="isCloseTable">
      ...
      >
        <generic-button
          ...
          type="primary"
          :margin="true"
          @click="$router.push('/preparePurchaseServiceInvoiceNew.htm')"
        />
        <div class="p-2">
          ...
          <GenericTablePage
            ...
            open-url="preparePurchaseServiceInvoiceNew"
            height="600"
          />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getTableRequest() {
      ...
      this.$axios
        .post(`/invoices/purchaseServiceInvoice`, {
          current_page: 1,
          page_size: this.pageSize_value,
          searchForm: {
            keyword: this.keywordValue,
            from_date: new Date(this.formData.from)
              .toLocaleString('en-GB')
              .split(',')
              .join(''),
            to_date: new Date(this.formData.to)
              .toLocaleString('en-GB')
              .split(',')
              .join(''),
          },
          billStatus: this.formData.bill,
          payStatus: this.formData.pay,
          departmentId: this.formData.departments,
