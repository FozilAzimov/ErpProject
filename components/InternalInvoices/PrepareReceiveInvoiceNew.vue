    ...
    >
      <div class="flex items-center gap-[10px]">
        <generic-button
          ...
          @click="$router.push('viabranchreceive.htm')"
        />
        <h1 class="font-bold text-[rgb(49,126,172)] text-[14px] uppercase">
          Receive - Internal Invoice
        </h1>
      </div>
    </div>
    <div>
        ...
        >
          ...
          <div class="flex gap-1 flex-wrap">
            ...
            <GenericButton
              ...
              @click="receiveUndoReceiveInvocieUrlRequestAction"
            />
            <GenericButton
              name="Print Preview"
              pl="8"
...
export default {
  ...
  methods: {
    ...
    getPageRequest() {
      ...
      this.$axios
        .post(
          `/invoices/prepareReceiveInvoiceNewAjaxLoad`,
          { id },
          {
            headers: {
        ...
        .then(({ data }) => {
          ...
          this.ignoredId = data?.invoiceJson?.warehouse?.id
    ...
    getLookUpValue(key, value, order) {
      ...
      this.lookupValuesObj.get('warehouseRef')
    ...
    // Undo recive btn action
    receiveUndoReceiveInvocieUrlRequestAction() {
      if (this.userId || this.parentID) {
        this.$axios.post(`/invoices/receiveUndoReceiveInvocieUrl`, {
          id: this.parentID ? Number(this.parentID) : Number(this.userId),
        })
      }
    },
    ...
    getResponseTransactionsAction(prop) {
      this.$axios
        .post(
          `/invoices/prepareInternalInvoiceNewAjaxLoad`,
    ...
    saveInvoice() {
      if (!this.disabledButton) {
        ...
        this.$axios
          .post(
            `/invoices/prepareCreateEditInternalInvoice`,
          ...
          .then(({ data, status }) => {
            ...
            if ((this.userId || this.parentID) && status === 200) {
              this.$router.push(
                `/prepareInternalInvoiceNew.htm/${this.parentID}`
              )
            }
          })
          .catch((error) => {
